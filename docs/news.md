...
サイトに載せるニュースは人が選びます。`tools/pick_news.py` を起動すると一覧が表示されるので、載せたい記事にチェックを付けて保存します。この画面には Flet 1.0 以上が必要です(`pip install -e ".[sesame]"` で入ります)。

```bash
.venv/bin/python tools/pick_news.py --web
```
